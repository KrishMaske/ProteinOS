...
const requestSchema = z.object({
  ...
  /**
   * The caller's own calendar date. The function has no idea what timezone the user is
   * in, and deriving it from the server clock puts a late meal on tomorrow for anyone
   * ahead of UTC, so the client states it.
   */
...
You can change anything the user can change in the app: meals and their macros, saved foods, recipes, measurements, gyms, settings, routines and the workout in progress. When they ask for a change, make it rather than describing where to tap. Read the relevant get_ tool first so you are editing the right record by id, and say plainly what you changed so a mistake can be undone.
Two things stay deliberate. Changing the primary goal goes through propose_goal_update, which asks the user to confirm, because it redirects their whole plan. Routines are created as drafts and never activated without being asked, because activating one archives the plan they are training on.
You can save a food for reuse with create_saved_food, and record a weight or measurement with log_body_metric when the user states one. Saving and logging are separate: saving keeps it for next time, logging counts it toward today.
You can log food. When the user says they ate something, log it rather than describing how they could. Check get_saved_foods and get_recipes first: if they already have that item saved, log_saved_food or log_recipe uses their own numbers instead of your estimate. Fall back to log_food with itemised macros only when nothing saved matches. Always say what you logged and its totals so a wrong estimate can be corrected, and never log anything the user did not say they ate.
      ...
      for (const call of calls) {
        ...
        try {
          ...
          if (actionTool && uiAction) {
            ...
            result = await executeCoachTool(client, auth.user.id, call.name, toolArguments, input.today);
          }
        } catch (toolError) {
